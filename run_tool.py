...
def blackbox(swagger, port,cov_port=None,service=None):
    timeout = time.time() + 60 * 60 * int(time_limit)
    while time.time() < timeout:
...
if __name__ == "__main__":
    ...
    if service == "features-service":
        if tool == "evomaster-whitebox":
            ...
            blackbox("file://$(pwd)/spec/features.yaml", 30100,cov_port=port,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/features.yaml"), 30100,cov_port=port,service=service)
    elif service == "languagetool":
        if tool == "evomaster-whitebox":
            whitebox(30100)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/languagetool.yaml", 30101,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/languagetool.yaml"), "30101/v2",cov_port=port,service=service)
    elif service == "ncs":
        if tool == "evomaster-whitebox":
            whitebox(30102)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/ncs.yaml", 30102,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/ncs.yaml"), 30102,cov_port=port,service=service)
    elif service == "restcountries":
        if tool == "evomaster-whitebox":
            whitebox(30106)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/restcountries.yaml", "30106",service=service)
        else:
            blackbox(os.path.join(curdir, "spec/restcountries.yaml"), "30106/rest",cov_port=port,service=service)
    elif service == "scs":
        if tool == "evomaster-whitebox":
            whitebox(30108)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/scs.yaml", 30108,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/scs.yaml"), 30108,cov_port=port,service=service)
    elif service == "genome-nexus":
        time.sleep(300)
        if tool == "evomaster-whitebox":
            whitebox(30110)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/genome.yaml", 30110,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/genome.yaml"), 30110,cov_port=port,service=service)
    elif service == "person-controller":
        if tool == "evomaster-whitebox":
            whitebox(30111)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/person.yaml", 30111,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/person.yaml"), 30111,cov_port=port,service=service)
    elif service == "user-management":
        if tool == "evomaster-whitebox":
            whitebox(30116)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/user.yaml", 30115,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/user.yaml"), 30115,cov_port=port,service=service)
    elif service == "market":
        if tool == "evomaster-whitebox":
            whitebox(30118)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/market.yaml", 30117,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/market.yaml"), 30117,cov_port=port,service=service)
    elif service == "project-tracking-system":
        if tool == "evomaster-whitebox":
            whitebox(30119)
        elif tool == "evomaster-blackbox":
            blackbox("file://$(pwd)/spec/project.yaml", 30118,service=service)
        else:
            blackbox(os.path.join(curdir, "spec/project.yaml"), 30118,cov_port=port,service=service)
